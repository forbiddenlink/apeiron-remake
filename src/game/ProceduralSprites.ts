...
export function drawMushroom(
  ...
  if (psychedelic) {
    ...
    capInner =
      profile === 'classic' ? (poisoned ? '#ff9442' : '#d83a67') : poisoned ? '#ef6e38' : '#4f76ad'
    capMid =
      profile === 'classic' ? (poisoned ? '#8c2517' : '#4a1c5e') : poisoned ? '#8a2416' : '#203b5d'
  }
  ...
  g.fillStyle = reflective
    ? 'rgba(255, 255, 255, 0.5)'
    : profile === 'classic'
      ? 'rgba(255, 245, 182, 0.35)'
      : 'rgba(233, 245, 255, 0.36)'
  ...
  if (reflective) {
    ...
    g.fillStyle = poisoned
      ...
      : profile === 'classic'
        ? 'rgba(255, 205, 218, 0.34)'
        : 'rgba(180, 223, 255, 0.34)'
    g.beginPath()
    g.ellipse(cx - 1.5, capY - 0.8, 1.65, 0.65, -0.22, 0, Math.PI * 2)
    g.fill()
  }
