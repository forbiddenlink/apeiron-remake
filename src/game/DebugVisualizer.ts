import { ENEMIES, GRID, PLAYER } from './GameConfig'
...
export class DebugVisualizer {
  ...
  draw() {
    ...
    if (this.settings.showSpawnZones) {
      ...
      this.ctx.fillRect(0, 0, GRID.COLS * GRID.CELL, GRID.CELL)

      // UFO spawn zone
      this.ctx.fillStyle = 'rgba(0, 229, 255, 0.1)'
      this.ctx.fillRect(0, 0, GRID.COLS * GRID.CELL, (GRID.ROWS * GRID.CELL) / 3)
    }
    ...
    if (this.settings.showCollisionPoints) {
      for (const point of this.collisionPoints) {
        ...
        const alpha = 1 - age
        this.ctx.fillStyle =
          point.type === 'hit' ? `rgba(255, 64, 129, ${alpha})` : `rgba(255, 255, 255, ${alpha})`
        this.ctx.beginPath()
    ...
    if (this.settings.showFPS || this.settings.showParticleCount) {
      ...
      this.ctx.fillRect(
        ...
        30,
        150,
        this.settings.showFPS && this.settings.showParticleCount ? 50 : 30
      )
