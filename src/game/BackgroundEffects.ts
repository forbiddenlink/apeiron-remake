...
export class BackgroundEffects {
  ...
  private static readonly THEMES: BackgroundTheme[] = [
    'field',
    'desert',
    'ocean',
    'neon',
  ...
  setWave(wave: number) {
    ...
    const themeIndex =
      Math.floor((wave - 1) / BackgroundEffects.WAVES_PER_THEME) % BackgroundEffects.THEMES.length
  ...
  private buildFieldCanvas(w: number, h: number, profile: VisualProfile): HTMLCanvasElement {
    ...
    for (let i = 0; i < (profile === 'classic' ? 400 : 280); i++) {
      ...
      tctx.ellipse(
        ...
        r * 1.25,
        r * 0.65,
        this.hash(i * 0.31, i * 1.91) * 0.5 - 0.25,
        0,
        Math.PI * 2
      )
      tctx.fill()
  ...
  private buildThemedCanvas(
    ...
    if (theme === 'neon') {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        tctx.moveTo(sx, sy)
        tctx.lineTo(
          sx + this.hash(i * 0.3, i * 2.1) * 100 - 50,
          sy + this.hash(i * 1.7, i * 0.5) * 100 - 50
        )
        tctx.stroke()
      }
