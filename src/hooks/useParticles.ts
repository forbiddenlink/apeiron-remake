...
export function useParticles({
  ...
  const triggerExplosion = useCallback(
    (
      ...
      const colors = Array.isArray(color)
        ...
        : color
          ? [color]
          : PARTICLE_COLOR_SCHEMES[colorScheme]
  ...
  const triggerTrail = useCallback(
    (x: number, y: number, color?: string | string[]) => {
      const colors = Array.isArray(color) ? color : color ? [color] : PARTICLE_COLOR_SCHEMES.energy
  ...
  const triggerPreset = useCallback(
    (x: number, y: number, preset: keyof typeof PARTICLE_PRESETS, colors?: string[]) => {
